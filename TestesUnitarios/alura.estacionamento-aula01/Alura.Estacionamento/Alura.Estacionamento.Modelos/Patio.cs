...
namespace Alura.Estacionamento.Modelos
{
    public class Patio
    {
        ...
        {
            var carro = (from veiculo in this.veiculos
                         where veiculo.Placa == veiculoAlterado.Placa
                         select veiculo).SingleOrDefault();
            carro.AlteraDados(veiculoAlterado);
            return carro;
        }
        ...
        public Veiculo PesquisaVeiculo(string idTIcket)
        {
            return  (from veiculo in this.veiculos
                              where veiculo._idTicket == idTIcket
                              select veiculo).SingleOrDefault();

        }
        private string GerarTicket(Veiculo veiculo)
        {
            veiculo._idTicket = new Guid().ToString().Substring(0,5);

            string ticket = "### Ticket MIlGrau##" +
                             $"ticket ID: {veiculo._idTicket}"+
                             $"Placa: {veiculo.Placa}";
